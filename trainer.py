...
from transformers import TrainingArguments, EvalPrediction, Trainer as Trainer_HF
...
class Trainer(Trainer_HF):
    ...
    def evaluation_loop(
        ...
        all_losses = np.zeros((len(dataloader), batch_size))
        all_preds = np.zeros((len(dataloader), batch_size, model.config.num_classes))
        all_labels = np.zeros((len(dataloader), batch_size))
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if loss is not None:
                ...
                all_losses[step, :] = np.array(losses.cpu())
            ...
            if logits is not None:
                ...
                all_preds[step, :] = np.array(logits[0].cpu())
            if labels is not None:
                labels = self.accelerator.pad_across_processes(
                    labels, dim=1, pad_index=-100
                )
                labels = self.gather_function((labels))
                all_labels[step, :] = np.array(labels.cpu())
        ...
        ):  # and  isinstance(eval_dataset, IterableDatasetShard):
